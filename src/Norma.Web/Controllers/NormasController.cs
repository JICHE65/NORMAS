...
namespace Norma.Web.Controllers
{
    public class NormasController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var request = await _httpClient.GetAsync($"http://localhost:5000/api/normas");
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var request = await _httpClient.GetAsync($"http://localhost:5000/api/normas/{id}");
            request.EnsureSuccessStatusCode();
        ...
        public async Task<IActionResult> Create(NormaVM model)
        {
            try
            {
                ...
                using var httpResponse =
                    await _httpClient.PostAsync($"http://localhost:5000/api/normas", modelJSON);
        ...
        public async Task<IActionResult> Edit(int id, NormaVM model)
        {
            try
            {
                ...
                using var httpResponse =
                    await _httpClient.PutAsync($"http://localhost:5000/api/normas/{id}", modelJSON);
                httpResponse.EnsureSuccessStatusCode();
            ...
            {
                ModelState.AddModelError("", "Sucedio un error en la modificación del registro" + ex.Message);
                return View(model);
            }
        }
        ...
        public async Task<IActionResult> Delete(int id, NormaVM model)
        {
            try
            {
                var request = await _httpClient.DeleteAsync($"http://localhost:5000/api/normas/{id}");
                request.EnsureSuccessStatusCode();
                return RedirectToAction(nameof(Index));
            }
            catch
        ...
        {
            var httpResponse = await _httpClient.GetAsync($"http://localhost:5000/api/requisitos");
            httpResponse.EnsureSuccessStatusCode();
